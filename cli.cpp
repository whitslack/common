...
namespace CLI {
	...
	std::string OptionException::make_what(const char msg[], const Option &opt) {
		std::string what(msg);
		if (opt.long_form) {
			what.append(": --").append(opt.long_form);
		}
		else {
			what.append(": -") += opt.short_form;
		}
		return what;
	}
	...
	bool StringOption::parse(char value[]) {
		...
		throw CLI::OptionException("expected argument for option", *this);
	}
			...
			if (arg[0] == '-') {
				if (arg[1] == '-') {
						...
						for (size_t j = 0; j < numopts; ++j) {
							...
							if (long_form && std::strncmp(long_form, arg, optlen) == 0 && long_form[optlen] == '\0') {
								if (arg[optlen] == '\0') {
									...
									throw CLI::OptionException("option takes no argument", *opts[j]);
								}
								goto next_arg;
							}
						}
						throw CLI::OptionException(std::string("unrecognized option: -") + (arg - 1));
					}
				}
				else if (arg[1] != '\0') {
					for (char *flag = arg + 1; *flag != '\0'; ++flag) {
						...
						throw CLI::OptionException(std::string("unrecognized option: -") + *flag);
					next_flag:;
					}
					goto next_arg;
				}
