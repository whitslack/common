...
int creat(const char *path, mode_t mode) {
	int ret;
	if (_unlikely((ret = ::creat(path, mode)) < 0)) {
...
int dup2(int fildes, int fildes2) {
	int ret;
	if (_unlikely((ret = ::dup2(fildes, fildes2)) < 0)) {
...
void fchownat(int fd, const char *path, uid_t owner, gid_t group, int flag) {
	if (_unlikely(::fchownat(fd, path, owner, group, flag) < 0)) {
...
int fcntl(int fildes, int cmd) {
	int ret;
	if (_unlikely((ret = ::fcntl(fildes, cmd)) == -1)) {
...
int fcntl(int fildes, int cmd, int arg) {
	int ret;
	if (_unlikely((ret = ::fcntl(fildes, cmd, arg)) == -1)) {
		throw std::system_error(errno, std::system_category(), "fcntl");
	}
	return ret;
}
...
int ioctl(int fildes, int request) {
	int ret;
	if (_unlikely((ret = ::ioctl(fildes, request)) == -1)) {
...
int ioctl(int fildes, int request, int arg) {
	int ret;
	if (_unlikely((ret = ::ioctl(fildes, request, arg)) == -1)) {
		throw std::system_error(errno, std::system_category(), "ioctl");
	}
	return ret;
}
...
bool isatty(int fildes) {
	int ret;
	errno = 0;
	if (_unlikely((ret = ::isatty(fildes)) == 0 && errno != ENOTTY)) {
...
off_t lseek(int fildes, off_t offset, int whence) {
	off_t ret;
	if (_unlikely((ret = ::lseek(fildes, offset, whence)) < 0)) {
...
void * mmap(void *addr, size_t len, int prot, int flags, int fildes, off_t off) {
	void *ret;
	if (_unlikely((ret = ::mmap(addr, len, prot, flags, fildes, off)) == MAP_FAILED)) {
...
int open(const char *path, int oflag, mode_t mode) {
	int ret;
	if (_unlikely((ret = ::open(path, oflag, mode)) < 0)) {
...
int openat(int fd, const char *path, int oflag, mode_t mode) {
	int ret;
	if (_unlikely((ret = ::openat(fd, path, oflag, mode)) < 0)) {
...
unsigned poll(struct pollfd fds[], nfds_t nfds, int timeout) {
	int ret;
	if (_unlikely((ret = ::poll(fds, nfds, timeout)) < 0)) {
		if (errno == EINTR) {
			return 0;
		}
...
ssize_t pread(int fildes, void *buf, size_t nbyte, off_t offset) {
	ssize_t ret;
	if (_unlikely((ret = ::pread(fildes, buf, nbyte, offset)) < 0)) {
...
ssize_t preadv(int fildes, const struct iovec iov[], int iovcnt, off_t offset) {
	ssize_t ret;
	if (_unlikely((ret = ::preadv(fildes, iov, iovcnt, offset)) < 0)) {
		if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
			return 0;
		}
...
size_t pwrite(int fildes, const void *buf, size_t nbyte, off_t offset) {
	ssize_t ret;
	if (_unlikely((ret = ::pwrite(fildes, buf, nbyte, offset)) < 0)) {
		if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
			return 0;
		}
...
size_t pwritev(int fildes, const struct iovec iov[], int iovcnt, off_t offset) {
	ssize_t ret;
	if (_unlikely((ret = ::pwritev(fildes, iov, iovcnt, offset)) < 0)) {
		if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
			return 0;
		}
...
ssize_t read(int fildes, void *buf, size_t nbyte) {
	ssize_t ret;
	if (_unlikely((ret = ::read(fildes, buf, nbyte)) < 0)) {
		if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
			return 0;
		}
...
ssize_t readv(int fildes, const struct iovec iov[], int iovcnt) {
	ssize_t ret;
	if (_unlikely((ret = ::readv(fildes, iov, iovcnt)) < 0)) {
		if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
			return 0;
		}
...
size_t readlink(const char * _restrict path, char * _restrict buf, size_t bufsize) {
	ssize_t ret;
	if (_unlikely((ret = ::readlink(path, buf, bufsize)) < 0)) {
...
size_t readlinkat(int fd, const char * _restrict path, char * _restrict buf, size_t bufsize) {
	ssize_t ret;
	if (_unlikely((ret = ::readlinkat(fd, path, buf, bufsize)) < 0)) {
...
void rename(const char *oldpath, const char *newpath) {
	if (_unlikely(::rename(oldpath, newpath) < 0)) {
...
void renameat(int oldfd, const char *oldpath, int newfd, const char *newpath) {
	if (_unlikely(::renameat(oldfd, oldpath, newfd, newpath) < 0)) {
		throw std::system_error(errno, std::system_category(), "renameat");
	}
}

void rmdir(const char *path) {
	if (_unlikely(::rmdir(path) < 0)) {
...
unsigned select(int nfds, fd_set * _restrict readfds, fd_set * _restrict writefds, fd_set * _restrict errorfds, struct timeval * _restrict timeout) {
	int ret;
	if (_unlikely((ret = ::select(nfds, readfds, writefds, errorfds, timeout)) < 0)) {
		if (errno == EINTR) {
			return 0;
		}
...
size_t write(int fildes, const void *buf, size_t nbyte) {
	ssize_t ret;
	if (_unlikely((ret = ::write(fildes, buf, nbyte)) < 0)) {
		if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
			return 0;
		}
...
size_t writev(int fildes, const struct iovec iov[], int iovcnt) {
	ssize_t ret;
	if (_unlikely((ret = ::writev(fildes, iov, iovcnt)) < 0)) {
		if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
			return 0;
		}
